...
"""
These functions are parts of the heuristic for the AlphaBeta player.
"""
...
@njit
def minimax_alpha_beta_cpp(player, state, depth, alpha=-np.inf, beta=np.inf, turn=0, score_heuristic=check_score):
    if depth == 0 or is_state_over(state, player):
        score = score_heuristic(player, state)
        return score
    if turn == 0:
        max_eval = -np.inf
...
class PAI:
    """
    This class is the AlphaBeta player.
    """

    def __init__(self, game, player, depth=4, gamma=0):
        self.game = game
        self.player = player
        self.depth = depth
        self.gamma = gamma
        self.name = "AB" + str(self.player) + " - " + str(self.depth)
    ...
    def get_winner_name(self):
        ...
        :return:
        """
        return "AlphaBeta" + str(self.player)
    ...
    def find_best_move(self, score_heuristic=check_score):
        """
        Find the best move using a heuristic.
        :param score_heuristic:
        :return:
        """
        ...
        for move in self.get_possible_moves(new_board):
            ...
            for row in range(len(new_state), 0, -1):
                if new_state[row - 1][move] == 0:
                    ...
                    moves.append([minimax_alpha_beta_cpp(self.player, new_state, self.depth - 1, turn=1), move])
